...
class DynamoDBHandler():
    ...
    def get_item(self, table_name:str, searchobj:dict):
        """consultar item"""
        response=[]

        dic = {f""":{key}""":{'S':val} for key ,val in searchobj.items()}
    ...
    def get_item_paginated(self,table_name:str, searchobj:dict):
        """consultar item"""
        response=[]
        page = 0
        limit = 10
        ...
        maxpages = math.ceil(len(responseitems)/limit)
        lastvec = responseitems[limit*(page-1):limit*page]
